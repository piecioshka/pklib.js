...
var Pklib = {
    ...
    Utils : {
        ...
        outerlink : function(place) {
            if ( Pklib.Validate.empty ( $(place) ) || typeof place === 'undefined' ) {
            	place = 'body';
            }
            $(place).find('a').each(function() {
                if ($(this).attr('rel') == 'outerlink') {
                    $(this).removeEvent('click').addEvent('click', function outerlink(e) {
    ...
    Message : {
         ...
         */
        show : function() {
            var message = $('<div />').addClass(this.objClass).css( {
				position : 'absolute',
				color : '#000000',
				width : 300,
				height : 200,
    ...
    Loader : {
        ...
        show : function(config) {
            ...
            loader.css({
                left: (center) ? ($(window).width() - loader.width() ) / 2 : 0,
                top: (center) ? ($(window).height() - loader.height() ) / 2 : 0 
            });
    ...
    Glass : {
        ...
        show : function(config) {
            ...
            var glassFrame = $('<iframe />').addClass(this.objClass).css( {
				height : maxHeight,
				width : maxWidth - 4,
				position : 'absolute',
				top : 0,
			...
			var glass = $('<div />').addClass(this.objClass).css( {
				height : maxHeight,
				width : maxWidth,
				position : 'absolute',
				top : 0,
