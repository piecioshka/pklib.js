/** pklib v1.0.4 | http://pklib.com/license **/
...
(function(global){"use strict";var pklib=global.pklib||{},cache=[];function XHRError(message){this.name="XHRError";this.message=message||"";}
...
return xhr;}}};}(this));(function(global){"use strict";var pklib=global.pklib||{};pklib.array={isArray:function(obj){return typeof obj==="object"&&obj!==null&&typeof obj.length!=="undefined"&&typeof obj.slice!=="undefined";},inArray:function(param,array){var i,len=array.length;for(i=0;i<len;++i){if(array[i]===param){return true;}}
...
if(c.indexOf(name)===0){return c.substring(name.length,c.length);}}},erase:function(name){return this.create(name,undefined,-1);}};}(this));(function(global){"use strict";var pklib=global.pklib||{};var rspace=/\s+/;var rclass=/[\n\t\r]/g;function checkParams(cssClass,element){if(Object(cssClass).constructor!==String){throw new TypeError("pklib.css.addClass: @cssClass: not String");}
if(!pklib.dom.isNode(element)){throw new TypeError("pklib.css.addClass: @element: not HTMLElement");}}
pklib.css={addClass:function(cssClass,element){checkParams(cssClass,element);var classElement=element.className;if(!this.hasClass(cssClass,element)){if(classElement.length){classElement+=" "+cssClass;}else{classElement=cssClass;}}
element.className=classElement;},removeClass:function(cssClass,element){checkParams(cssClass,element);var trim=pklib.string.trim;var classNames=(element.className||"").split(rspace);if(classNames){var className=(" "+element.className+" ").replace(rclass," ");for(var c=0,cl=classNames.length;c<cl;c++){className=className.replace(" "+classNames[c]+" "," ");}
element.className=trim(className);}else{element.className="";}},hasClass:function(cssClass,element){checkParams(cssClass,element);var className=" "+cssClass+" ";return((" "+element.className+" ").replace(rclass," ").indexOf(className)>-1);}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{};function walk_the_dom(node,func){func(node);node=node.firstChild;while(node){walk_the_dom(node,func);node=node.nextSibling;}}
...
return array;},center:function(element,wrapper){var left=null,top=null,pus=pklib.utils.size;if(!this.isElement(element)){throw DOMException();}
...
element.style.left=left+"px";element.style.top=top+"px";element.style.position="absolute";return[left,top];},maximize:function(element,wrapper){var width=null,height=null,pus=pklib.utils.size;if(wrapper===document.body){width=Math.max(pus.window("width"),pus.document("width"));height=Math.max(pus.window("height"),pus.document("height"));if(pklib.browser.getName()==="msie"){width-=20;}}else{width=pus.object(wrapper,"width");height=pus.object(wrapper,"height");}
element.style.width=width;element.style.height=height;return[width,height];},insert:function(element,node){if(this.isNode(element)){node.appendChild(element);}else if(typeof element==="string"){node.innerHTML+=element;}
...
target.events[eventName].push(handler);if(target.attachEvent){target.attachEvent("on"+eventName,handler);}else if(target.addEventListener){target.addEventListener(eventName,handler,false);}},remove:function(target,eventName,handler){if(typeof target.events==="undefined"){target.events={};}
var removeEvent;if(target.detachEvent){removeEvent="detachEvent";}else if(target.removeEventListener){removeEvent="removeEventListener";}
var events=target.events[eventName];if(typeof events!=="undefined"){var len=events.length;for(var i=0;i<len;++i){var handler=events[i];target[removeEvent](eventName,handler);delete target.events[eventName];}}},get:function(target,eventName){if(typeof target.events==="undefined"){target.events={};}
return target.events[eventName];},trigger:function(target,eventName){if(typeof target.events==="undefined"){target.events={};}
var events=target.events[eventName];if(typeof events!=="undefined"){var len=events.length;for(var i=0;i<len;++i){events[i].call(target,events[i]);}}else{throw new ReferenceError("pklib.event.trigger: @event "+eventName+": undefined");}}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{},lazy_file=0;function loadjs(url,callback){var script=document.createElement("script");script.type="text/javascript";script.src=url;if(script.readyState){script.onreadystatechange=function(){if(script.readyState==="loaded"||script.readyState==="complete"){script.onreadystatechange=null;if(typeof callback==="function"){callback(script);}}};}else{script.onload=function(){if(typeof callback==="function"){callback(script);}};}
if(typeof document.head==="undefined"){document.head=document.getElementsByTagName("head")[0];}
document.head.appendChild(script);}
pklib.file={load:function(src,callback){loadjs(src,function(script){if(typeof callback==="function"){callback(script);}});},lazy_load:function(files,callback,is_continue){var that=this,len=files.length,file=files[lazy_file];if(!is_continue){lazy_file=0;}
loadjs(file,function(){if(lazy_file<len-1){lazy_file+=1;that.lazy_load(files,callback,true);}else{if(typeof callback==="function"){callback();}}});}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{},id="pklib-glass-wrapper",settings={container:null,style:{position:"absolute",left:0,top:0,background:"#000",opacity:0.5,zIndex:1000}};pklib.glass={objId:id,show:function(config,callback){var that=this,glass=document.createElement("div"),glassStyle=glass.style,style;settings.container=document.body;settings=pklib.array.mixin(settings,config);settings.style.filter="alpha(opacity="+parseFloat(settings.style.opacity,10)*100+")";glass.setAttribute("id",this.objId);for(style in settings.style){if(settings.style.hasOwnProperty(style)){glassStyle[style]=settings.style[style];}}
settings.container.appendChild(glass);pklib.dom.maximize(glass,settings.container);pklib.event.add(global,"resize",function(){that.close();that.show(config,callback);pklib.dom.maximize(glass,settings.container);});if(typeof callback==="function"){callback();}
...
return result;}};}(this));(function(global){"use strict";var pklib=global.pklib||{};function getFunctionName(fun){var text=fun.toString().split("\n")[0].replace("function ","");return text.substr(0,text.indexOf("("))+"()";}
...
if(typeof callback==="function"){callback();}
return result;}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{},id="pklib-message-wrapper",settings={container:null,style:{width:300,height:300,zIndex:1010}};pklib.message={objId:id,content:null,show:function(config,callback){settings.container=document.body;settings=pklib.array.mixin(settings,config);var message=document.createElement("div"),messageStyle=message.style,style;message.setAttribute("id",this.objId);for(style in settings.style){if(settings.style.hasOwnProperty(style)){messageStyle[style]=settings.style[style];}}
...
settings.container.appendChild(message);pklib.dom.center(message,settings.container);pklib.event.add(global,"resize",function(){pklib.dom.center(message,settings.container);});if(typeof callback==="function"){callback();}
...
break;}}
return text;}};}(this));(function(global){"use strict";var pklib=global.pklib||{},loc=global.location;pklib.url={getProtocol:function(){return loc.protocol;},getHost:function(){return loc.host;},getPort:function(){return loc.port||80;},getUri:function(){return loc.pathname;},getParams:function(){var params=loc.search,params_obj={},i,item,len=0;if(params.substr(0,1)==="?"){params=params.substr(1);}
...
params=params.split("&");for(i=0;i<len;++i){item=params[i].split("=");if(item[0]===key){return item[1];}}},getHash:function(){return loc.hash;}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{};pklib.utils={size:{window:function(name){var clientName;if(typeof name==="undefined"){throw new TypeError("pklib.utils.size.window: @name: undefined");}
name=pklib.string.capitalize(name);clientName=global.document.documentElement["client"+name];return(global.document.compatMode==="CSS1Compat"&&clientName)||global.document.body["client"+name]||clientName;},document:function(name){var clientName,scrollBodyName,scrollName,offsetBodyName,offsetName;if(typeof name==="undefined"){throw new TypeError("pklib.utils.size.document: @name: undefined");}
