...
if(c.indexOf(name)===0){return c.substring(name.length,c.length);}}},erase:function(name){return this.create(name,undefined,-1);}};}(this));(function(global){"use strict";var pklib=global.pklib||{},rspace=/\s+/,rclass=/[\n\t\r]/g;function checkParams(cssClass,element){if(typeof cssClass!=="string"){throw new TypeError("pklib.css.addClass: @cssClass: not String");}
...
element.className=classElement;},removeClass:function(cssClass,element){checkParams(cssClass,element);var trim=pklib.string.trim,c=0,className="",classNames=(element.className||"").split(rspace),cl=classNames.length;if(classNames){className=(" "+element.className+" ").replace(rclass," ");for(c=0;c<cl;c++){className=className.replace(" "+classNames[c]+" "," ");}
...
break;}}
return text;}};}(this));(function(global){"use strict";var pklib=global.pklib||{},loc=global.location;pklib.url={getProtocol:function(){return loc.protocol;},getHost:function(){return loc.host;},getPort:function(){return loc.port||80;},getUri:function(){return loc.pathname;},getParams:function(){var i,item,len=0,params=loc.search,params_obj={};if(params.substr(0,1)==="?"){params=params.substr(1);}
