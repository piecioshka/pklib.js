...
if(c.indexOf(name)===0){return c.substring(name.length,c.length);}}},remove:function(name){return this.create(name,undefined,-1);}};}(this));(function(global){"use strict";var pklib=global.pklib||{},rspace=/\s+/,rclass=/[\n\t\r]/g;function checkParams(cssClass,element){if(typeof cssClass!=="string"){throw new TypeError("pklib.css.addClass: @cssClass: not String");}
...
return array;},insert:function(element,node){if(this.isNode(element)){node.appendChild(element);}else if(typeof element==="string"){node.innerHTML+=element;}
...
pklib.file={loadjs:function(files,callback){if(typeof files==="string"){var src=files;loadjs(src,function(script){if(typeof callback==="function"){callback(script);}});}else{var that=this,len=files.length,file=files[lazy_file];if(!is_continue){lazy_file=0;}
loadjs(file,function(){if(lazy_file<len-1){lazy_file+=1;that.lazyLoad(files,callback,true);}else{if(typeof callback==="function"){callback();}}});}}};}(this));(function(global){"use strict";var pklib=global.pklib||{};function getFunctionName(fun){var text=fun.toString().split("\n")[0].replace("function ","");return text.substr(0,text.indexOf("("))+"()";}
...
element.style.width=width;element.style.height=height;return[width,height];}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{},id="pklib-loader-wrapper",settings={src:"http://pklib.com/img/icons/loader.gif",container:null,style:{width:31,height:31,zIndex:1010},center:true};pklib.ui.loader={objId:id,show:function(config,callback){settings.container=document.body;settings=pklib.array.mixin(settings,config);var loader=document.createElement("img"),loaderStyle=loader.style,style;loader.setAttribute("id",this.objId);loader.setAttribute("src",settings.src);for(style in settings.style){if(settings.style.hasOwnProperty(style)){loaderStyle[style]=settings.style[style];}}
...
if(typeof callback==="function"){callback();}
return result;}};}(this));(function(global){"use strict";var pklib=global.pklib||{},document=global.document||{},id="pklib-glass-wrapper",settings={container:null,style:{position:"absolute",left:0,top:0,background:"#000",opacity:0.5,zIndex:1000}};if(typeof pklib.ui==="undefined"){pklib.ui={};}
...
if(typeof callback==="function"){callback();}
return result;}};}(this));(function(global){"use strict";var pklib=global.pklib||{};pklib.ui.size={window:function(name){var clientName;if(typeof name==="undefined"){throw new TypeError("pklib.ui.size.window: @name: undefined");}
...
name=pklib.string.capitalize(name);var client=obj["client"+name],scroll=obj["scroll"+name],offset=obj["offset"+name];return Math.max(client,scroll,offset);}};}(this));